...
- **Short links**: Optional `redirects.json` for `/r/` — browser redirects or static proxy files for `curl` (see Get Started)
...
- ✅ **Short redirects**: `/r/` from `redirects.json` — browser redirects (`delay`) or baked-in proxy files (`proxy: true` / raw URLs)
