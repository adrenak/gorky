...
(function () {
    ...
    function tagPathSegment(tag) {
        tag = String(tag || '').trim();
        if (!tag) return 'tag';
        if (tag.length <= 96 && /^[a-zA-Z0-9_-]+$/.test(tag)) return tag;
        var s = encodeURIComponent(tag).replace(/%/g, '');
        return s ? s.slice(0, 96) : 'tag';
    }
    ...
    if (params.has('post')) {
        ...
        if (tag && tag.toLowerCase() === 'all') {
            ...
            target = root + 'posts/tag/' + tagPathSegment(tag) + '/';
        }
    }
