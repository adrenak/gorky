...
const { extractPostMetadata, encodeTagForPath } = require('./posts');
    ...
    if (metadata.tags) {
        ...
        if (tags.length > 0) {
            ...
            html += tags.map((tag, index) => {
                ...
                return `<a href="${resolveSitePath(rootPrefix, `posts/tag/${encodeTagForPath(tag)}/`)}" class="tag-link">${escapeHtml(tag)}</a>${separator}`;
            }).join('');
            html += '</div>\n';
        }
    }
...
/**
 * Compact mobile top bar (hamburger, favicon, site name).
 * @param {Object} siteConfig
 * @param {string} rootPrefix
 * @returns {string}
 */
function generateMobileTopbar(siteConfig, rootPrefix = '') {
    const siteName = escapeHtml(siteConfig.siteName || 'Site');
    const homeHref = escapeHtml(resolveSitePath(rootPrefix, './'));
    let faviconPath = (siteConfig.favicon && String(siteConfig.favicon).trim()) || 'favicon.ico';
    if (!faviconPath.startsWith('http://') && !faviconPath.startsWith('https://')) {
        faviconPath = resolveSitePath(rootPrefix, faviconPath.replace(/^\//, ''));
    }
    const faviconSrc = escapeHtml(faviconPath);

    return `<header class="mobile-topbar" id="mobile-topbar" aria-hidden="true">
  <button type="button" class="mobile-menu-toggle" id="hamburger-menu" aria-label="Open menu" aria-expanded="false" aria-controls="sidebar">
    <span></span>
    <span></span>
    <span></span>
  </button>
  <a class="mobile-topbar-brand" href="${homeHref}">
    <img class="mobile-topbar-favicon" src="${faviconSrc}" alt="" width="22" height="22" />
    <span class="mobile-topbar-title">${siteName}</span>
  </a>
</header>
`;
}
...
function assemblePage(template, options) {
    ...
    const mobileTopbarHTML = generateMobileTopbar(siteConfig, rootPrefix);
    ...
    html = html
        ...
        .replace('{{MOBILE_TOPBAR}}', mobileTopbarHTML)
