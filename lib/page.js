...
function generateStructuredData({ title, description, url, metadata, siteConfig, isPost }) {
    ...
    if (isPost && metadata.date) {
        ...
        if (metadata.thumbnail) {
            schema.image = metadata.thumbnail;
        }
        if (metadata.tags) {
...
function assemblePage(template, options) {
    ...
    const prefixedContentHTML = prefixContentAssetPaths(contentHTML, assetPrefix);
    ...
    let html = template;
    html = replaceAll(html, '{{ASSET_PREFIX}}', assetPrefix);
    html = html
        .replace('{{SITE_CONFIG}}', siteConfigJson)
        .replace('{{ASSET_PREFIX_JSON}}', JSON.stringify(assetPrefix))
