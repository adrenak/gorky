 ...
 */
function generatePostsMd(postsPath, postsMdPath, contentDir = 'content', postLinkPrefix = '../') {
    ...
    mdFiles.forEach(file => {
        ...
        // Skip archived posts (still accessible via direct ?post= URL)
        ...
        const thumbnailPath = processThumbnailPath(frontmatter.data.thumbnail, `${contentDir}/posts/`);
    ...
    if (distinctTags.length > 0) {
        postsBody += '## Tags\n\n';
        postsBody += distinctTags.map(tag => {
    ...
    posts.forEach(post => {
        ...
        if (post.thumbnail) {
            ...
            const escapedTags = escapeHtmlAttribute(post.tags || '');
            postsBody += `<div class="post-entry"${escapedTags ? ` data-tags="${escapedTags}"` : ''}>\n`;
            postsBody += `<img src="${post.thumbnail}" alt="${escapedTitle}" class="post-thumbnail" />\n`;
            postsBody += `<div class="post-content">\n`;
            postsBody += `<h2><a href="${postLinkPrefix}post/${post.slug}/">${post.title}</a></h2>\n`;
            if (post.description) {
                postsBody += `<p>${post.description}</p>\n`;
            }
            postsBody += `</div>\n</div>\n\n`;
        } else {
            // For posts without thumbnails, use markdown
            postsBody += `## [${post.title}](${postLinkPrefix}post/${post.slug}/)\n\n`;
            if (post.description) {
                postsBody += `${post.description}\n\n`;
            }
        }
    });

    const postsMd = intro
        ? matter.stringify(postsBody, buildPostsPageFrontmatter(intro.data))
        : postsBody;
