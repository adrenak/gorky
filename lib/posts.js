...
function generatePostsMd(postsPath, postsMdPath, contentDir = 'content', postLinkPrefix = '../') {
    ...
    // Add tags section with counts (All first — resets tag filter on the posts page)
    if (distinctTags.length > 0) {
        postsBody += '## Tags\n\n';
        const allLink = `[All (${posts.length})](?tag=all)`;
    ...
    posts.forEach(post => {
        ...
        postsBody += `<div class="post-entry"${dataTagsAttr}>\n`;

        if (post.thumbnail) {
            postsBody += `<img src="${escapeHtml(post.thumbnail)}" alt="${escapeHtml(post.title)}" class="post-thumbnail" />\n`;
        }

        postsBody += `<div class="post-content">\n`;
        postsBody += `<h2><a href="${escapeHtml(postHref)}">${escapeHtml(post.title)}</a></h2>\n`;
        if (post.description) {
            postsBody += `<p>${escapeHtml(post.description)}</p>\n`;
        }
        postsBody += `</div>\n</div>\n\n`;
    });
